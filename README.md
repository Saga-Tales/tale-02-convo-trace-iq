...
| 2 | 시나리오 + 회화 — Claude Haiku 4.5 + hard-constraint validator + streaming + CEFR 9단계 + 풍부한 시나리오 (brief·objectives·keyExpressions·learningGoals) |
| 3 | 캡처 (페인 1·2) — ko→en 모달 + 단어 lookup 모달 + 자동 contextSentence + 복합 unique 인덱스 dedup |
| 4 | 자동 추출 (페인 3) + 디자인 v2 — 종료 후 LLM이 transcript에서 좋은 표현 추출 + keep/discard UI + vibrant pink 리뉴얼 |
| 5 | **회상 폐쇄 루프 (페인 4)** — selectRecallPhrases + scenario.ts hint 주입 + substring keyword match로 자동 mastery 업데이트 + SM-2 단순화 + Home 정교화 |
| 6 | **N명 페어 + QR 동기화** — sessionUuid·role·participants (v4 마이그레이션) + pako gzip + 호스트 중심 모델 + 5개 새 컴포넌트 |
| 7 | **PWA + 백업** — vite-plugin-pwa + manifest + 아이콘 (cairosvg) + iOS Safari 메타태그 + JSON export/import (Merge/Replace) |

**총 86 modules → 176 modules, gzip 161KB.** 7일 동안 핵심 가치 4가지 모두 구현 + 페어 동기화 + 휘발성 대비까지 완성.
...
- **Dogfooding 사이클** — 우테코 동아리 영어 회화 스터디에서 실제 사용. 페어 동기화 흐름의 자연스러움, QR 사이즈 한계, mastery 업데이트의 precision 등 검증.
- **Saga-Tales promotion 검토** — venture studio의 promotion 기준 (3-of-4) 평가.
- **마이크로 fix** — dogfooding에서 발견된 어색함 수정.
