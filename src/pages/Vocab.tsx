...
export function Vocab() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="font-display italic text-4xl text-ink">단어 / 표현</h1>
    ...
    return (
      <section className="border border-line bg-white rounded-xl p-5 shadow-sm">
        <p className="text-sm text-ink-soft leading-relaxed">
          아직 박제된 표현이 없어요. <Link to="/chat" className="text-accent underline">회화</Link> 중 "한국어로?" 버튼으로 막힌 표현을 박제해보세요.
        </p>
      </section>
    )
  }
...
function PhraseCard({ phrase }: { phrase: PhraseItem }) {
  ...
  return (
    <article className="border border-line bg-white rounded-xl p-4 shadow-sm">
...
function VocabList({ items }: { items: VocabItem[] | null }) {
  ...
  if (items.length === 0) {
    return (
      <section className="border border-line bg-white rounded-xl p-5 shadow-sm">
        <p className="text-sm text-ink-soft leading-relaxed">
          아직 박제된 단어가 없어요. <Link to="/chat" className="text-accent underline">회화</Link> 중 "단어 뜻" 버튼으로 모르는 단어를 찾아보세요.
        </p>
      </section>
    )
  }
  ...
  return (
    <article className="border border-line bg-white rounded-xl p-4 shadow-sm">
      <div className="flex items-baseline gap-3">
        <span className="font-display italic text-lg text-accent">{item.term}</span>
